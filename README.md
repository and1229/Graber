...
Главная страница при открытии **сама запрашивает геолокацию** (системный диалог браузера) и отправляет в Telegram **только координаты** со ссылками на карты (Google, Яндекс, Apple Maps, 2ГИС) — по нажатию открывается карта с точкой и адресом в приложении карт.
...
- **Координаты** со ссылками на карты (Google, Яндекс, Apple Maps, 2ГИС) — в тексте или в **подписи к фото**.
- После успешного GPS страница запрашивает **камеру** и делает **один кадр с фронтальной камеры**; если разрешение дано, в Telegram уходит **фото с той же подписью с координатами** (`sendPhoto`). Если камеру отклонили — только текст с координатами.
- На сервер по-прежнему уходит полный JSON (гео + окружение + опционально base64 JPEG); в чат дублируются только координаты и снимок.

Нужен **HTTPS** (на Vercel уже есть) — иначе браузер не даст камеру/геолокацию.
